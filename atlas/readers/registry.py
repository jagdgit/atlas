"""Reader Registry (Phase B · §B.4 / OI-C1–C2 — lives under ``atlas.readers``).

A **Reader** turns an Asset into a structured Artifact for one family of inputs. The
...
**declared**, Atlas fails *honestly*. Code, media, and document readers all register here (OI-C2).

``atlas.engineering.readers`` re-exports this module for back-compat.
"""
...
CAP_AUDIO = "audio"
# Document / prose axis (OI-C2).
CAP_SECTIONS = "sections"
CAP_TABLES = "tables"
...
def default_document_readers() -> list[Reader]:
    """Document / conversation prose readers (OI-C2) — text/sections axis, not code caps."""
    return [
        Reader(
            id="document",
            ...
            languages=("document", "prose"),
            coverage={
                CAP_TEXT: True,
                CAP_SECTIONS: True,
                CAP_TABLES: False,
                CAP_METADATA: False,
                CAP_TRANSCRIPT: False,
                CAP_AUDIO: False,
            },
            priority=75,
        ),
...
class ReaderRegistry:
    ...
    def __init__(
        ...
        *,
        ...
        self._logger = logger or logging.getLogger("atlas.readers.registry")
        if readers is None:
            readers = [
                *default_readers(),
    ...
    def extension_map(self) -> dict[str, str]:
        ...
        out: dict[str, str] = {}
        for reader in reversed(self.readers(enabled_only=True)):
