"""Derived Artifact Store (Phase B · §B.2 / OI-C1 — neutral ``atlas.artifacts`` package).

Readers turn an **Asset** into a structured **Artifact** (AST / symbol table / repo map /
dependency graph / document text) *before* knowledge extraction: **Asset → Reader → Artifact →
Extraction → Knowledge**. Those artifacts are **deterministic derived products**, not throwaway
cache entries, so they are kept in this **Derived Artifact Store**, keyed by
``{asset_id, asset_version, reader, reader_version}``. Improving the *extractor* later re-runs
extraction against the stored artifact **without re-parsing** the source. The store is
regenerable/derived — the Asset stays the source of truth — so its physical backing (a Storage
cache scope here) is an implementation detail.

Moved out of ``atlas.engineering`` (OI-C1) so non-code readers do not couple to engineering.
``atlas.engineering.artifacts`` re-exports this module for back-compat.
"""
...
class DerivedArtifactStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._logger = logger or logging.getLogger("atlas.artifacts.store")
