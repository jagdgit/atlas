"""PersonalMentorWorker — Personal Intelligence mentor (closes Program stub).

Periodically synthesizes personal/career Experiences into a mentor Lesson written
back to Experience OS. Soft-bias (default on) teaches future ``advice_for`` recall.
"""
...
class PersonalMentorWorker(PersistentWorker):
    ...
    def __init__(
        ...
        *,
        ...
        self._events = events
        self._logger = logger or logging.getLogger("atlas.workers.personal_mentor")
    ...
    def do_tick(self, ctx: TickContext) -> TickResult:
        ...
        focus = str(cfg.get("focus") or "personal").strip() or "personal"
        ...
        lesson = synthesize_personal_lesson(
        ...
        if lesson is None:
            return TickResult(
                state=state,
                note=(
                    "mentor idle: no personal Experiences yet — run owner_knowledge "
        ...
        return TickResult(
            state=state,
            note=f"mentor[{focus}]: wrote={wrote} {lesson.title[:50]} | {rec}",
        )
