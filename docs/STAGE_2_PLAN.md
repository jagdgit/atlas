...
>   observability, e2e test, S22)**
> + **Web Console (zero-build same-origin SPA: chat + jobs + system, S23)**; 779 tests).
> Plan finalized (D1–D13, R1–R4; Q1–Q10 resolved).
> Next: **Stage 2 is complete** (incl. the Web UI). Optional follow-ons only: deeper
...
| **S23** ✅ | **Web Console** | A **zero-build, dependency-free** single-page app (vanilla HTML/CSS/JS in `atlas/web/static/`) served **same-origin** by the API (`mount_ui` → `/ui`, `/`→`/ui/`, gated by `api.ui_enabled`), so there's no CORS and no Node toolchain. **API-key login** (validated against `/v1/status`, kept in `localStorage`, `Bearer` on every call); three views — **Chat** (sessions sidebar + history + streaming-style send, tool-call chips, capability-gap warnings, citations with links), **Jobs** (create + list + live-polling detail with steps/progress/blocked + resume/cancel + rendered report), **System** (`/v1/status` cards + `/v1/health` per-service severity badges). XSS-safe DOM (textContent-only). 779 tests. **Conversational frontend delivered.** | visual chat + ops surface |
| **Web UI** *(spec)* | **Conversational frontend** | local frontend over REST (auth/CORS ready); can be pulled forward after S10 if a visual chat surface is wanted sooner — **delivered as S23** | — |
...
### 6t. Sprint 23 — Web Console ✅

The long-planned "conversational frontend" — a visual surface over the REST API so
Atlas isn't CLI/HTTP-only. Built to match the codebase's zero-heavy-deps ethos.

**Stack decision.** A **zero-build, dependency-free SPA** (vanilla HTML/CSS/JS) served
by the *same* FastAPI app as the API. No React/Vite/Node toolchain, no build step, and
because it's **same-origin** there is **no CORS** to configure. This mirrors earlier
choices (argparse over Click, in-memory metrics over a Prometheus client).

**What shipped**
- **Serving.** `atlas/web/__init__.py` `mount_ui(app, config)` mounts `atlas/web/static/`
  at `/ui` (`StaticFiles(html=True)`) and redirects `/`→`/ui/`, gated by a new
  **`api.ui_enabled`** flag; disabled ⇒ the mount and the root redirect are both absent.
- **Auth.** The shell is public (holds no secrets); the JS asks for the operator's API
  key once, validates it against `GET /v1/status`, keeps it in `localStorage`, and sends
  it as `Bearer` on every `/v1` call. A 401 anywhere forces re-login; "sign out" clears it.
- **Chat view.** Sessions sidebar (`/v1/chat/sessions`) + "New chat"; clicking a session
  loads its history (`/v1/chat/sessions/{id}`); the composer posts to `/v1/chat` (Enter to
  send, Shift+Enter for newline) and renders the assistant turn with **tool-call chips**,
  **capability-gap** warnings, and **citations** (linked when a source URL exists).
- **Jobs view.** Create an async job (`POST /v1/jobs`), list jobs, and open a detail pane
  that **live-polls** while `queued`/`running` — steps table, progress, blocked items,
  **Resume**/**Cancel** actions, and the finalized **report** rendered inline.
- **System view.** `/v1/status` summary cards (version/uptime/severity counts) + `/v1/health`
  per-service **severity badges** (ok/degraded/failed) — the S22 operability work made visible.
- **Safety.** All rendering is `textContent`-only via a small `el()` helper (no `innerHTML`
  with server data), so tool output / titles / snippets can't inject markup.

**Definition of Done (S23)** — all met:
- [x] `api.ui_enabled` config + default; `atlas/web/` package (`mount_ui`/`static_dir`) wired into `create_app`.
- [x] `index.html` + `styles.css` + `app.js` (login, chat, jobs, system; same-origin; XSS-safe DOM).
- [x] `/ui` served (public shell), `/`→`/ui/`; `ui_enabled=false` removes both.
- [x] Assets ship in the wheel (hatchling includes package data) — no build step, no new runtime deps.
- [x] Tests (assets on disk, index+assets served, root redirect, disabled→404, API unshadowed). **779 tests pass (+6).**
...
| Date | Sprint | Notes |
|------|--------|-------|
| 2026-07-12 | S23 | **Sprint 23 shipped ✅ — Web Console (the conversational frontend).** A **zero-build, dependency-free** single-page app (vanilla HTML/CSS/JS under `atlas/web/static/`) served **same-origin** by the FastAPI app — no React/Node build step and **no CORS**, consistent with Atlas's zero-heavy-deps ethos (argparse over Click, in-memory metrics over a Prometheus client). `atlas/web/__init__.py` **`mount_ui(app, config)`** mounts the assets at **`/ui`** (`StaticFiles(html=True)`) and redirects `/`→`/ui/`, gated by a new **`api.ui_enabled`** flag (off ⇒ mount + redirect both absent). **Auth:** the shell is public (no secrets); the JS collects the operator's API key once, validates it against `GET /v1/status`, keeps it in `localStorage`, sends it as `Bearer` on every `/v1` call, and forces re-login on any 401. **Three views:** **Chat** (sessions sidebar + `/v1/chat/sessions{,/{id}}` history, composer → `POST /v1/chat`, tool-call chips, capability-gap warnings, linked citations), **Jobs** (`POST /v1/jobs` create + list + a **live-polling** detail pane with steps/progress/blocked, **Resume**/**Cancel**, inline finalized report), **System** (`/v1/status` cards + `/v1/health` per-service **severity badges** — surfacing the S22 operability work). All rendering is `textContent`-only via a small `el()` helper (XSS-safe). Assets ship in the wheel (hatchling package data) with **no new runtime dependency**. Tests: assets on disk, index+assets served, `/`→`/ui/` redirect, `ui_enabled=false`→404, API not shadowed. **779 tests pass (+6).** **Stage 2 is complete, Web UI included.** |
