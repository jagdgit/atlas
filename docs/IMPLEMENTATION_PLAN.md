...
│   │   ├── preflight.py      # PREFLIGHT CHECKS (Stage 2 S22) — config validation + DB/LLM
│   │   │                     #   probes without starting; powers `atlas doctor`
...
│   │   ├── sql_plugin.py     # sql.query/tables/schema (read-only)— Stage 2 S20b
│   │   ├── ocr_plugin.py     # ocr.image (image→text, graceful)   — Stage 2 S20c
│   │   ├── mail_plugin.py    # mail.search/message/folders (RO IMAP)— Stage 2 S20d
│   │   └── browser_plugin.py # browser.open/screenshot (RO headless)— Stage 2 S20e
│   │
...
│   │
│   ├── sql/                  # READ-ONLY SQL (Stage 2 S20b) ✅
│   │   └── client.py         # SQLClient + SQLBackend/SQLiteBackend (read-only guard +
│   │                         #   mode=ro; sandboxed sources; query/tables/schema; honest outcomes)
│   │
│   ├── ocr/                  # OCR image→text (Stage 2 S20c) ✅
│   │   └── engine.py         # OCRClient + OCREngine/TesseractEngine seam (lazy deps,
│   │                         #   degrades gracefully; sandboxed sources; honest outcomes)
│   │
│   ├── mail/                 # READ-ONLY EMAIL (Stage 2 S20d) ✅
│   │   └── client.py         # MailClient + MailBackend/IMAPBackend seam (readonly=True +
│   │                         #   BODY.PEEK, no write path; degrades gracefully; honest outcomes)
│   │
│   ├── browser/              # HEADLESS BROWSER (Stage 2 S20e) ✅
│   │   └── browser.py        # BrowserClient + BrowserBackend/PlaywrightBackend seam (read-only
│   │                         #   render+extract; lazy deps→graceful unavailable; robots+sandbox)
│   │
│   ├── research/             # AUTONOMOUS RESEARCH ORCHESTRATION (Stage 2 S21) ✅
│   │   └── service.py        # ResearchService (research cap): gather→verify→decide loop over one
│   │                         #   EvidenceGraph claim; pure query_plan/extract_value; lazy providers;
│   │                         #   stops on convergence (not count) → verified report; honest outcomes
│   │
│   ├── ops/                  # OPERATIONS (Sprint 9) ✅
│   │   └── backup.py         # BackupManager (pg_dump + retention, ADR-0055)
│   │
│   ├── api/                  # REST API (FastAPI, Sprint 5) ✅
│   │   ├── app.py            # create_app (lifespan, error handlers, CORS, request obs, /ui mount)
│   │   ├── server.py         # uvicorn serve()
│   │   ├── routes.py         # public /health /metrics + authed /v1/* (+ /v1/status, S22)
│   │   ├── auth.py           # require_api_key (Bearer, fail-closed)
│   │   └── schemas.py        # Pydantic request/response models
│   │
│   ├── web/                  # WEB CONSOLE (Stage 2 S23) ✅ — zero-build same-origin SPA
│   │   ├── __init__.py       # mount_ui(app, cfg): /ui StaticFiles + /→/ui/ (gated by api.ui_enabled)
│   │   └── static/           # index.html + styles.css + app.js (chat / jobs / system; vanilla JS)
│   │
...
│   │                         #   websearch/download/scholar/youtube/code/python/git/sql/report/
...
| **Sprint 20b** | SQL (read-only) | `atlas/sql/client.py` `SQLClient` + injectable `SQLBackend`/`SQLiteBackend` (read-only guard `is_read_only` + `mode=ro`; sandboxed sources; row cap + soft timeout; honest outcomes); `SQLPlugin` (`sql` cap: `query`/`tables`/`schema`); `SQLCapability` (`CAP_SQL`); `plugins.sql.*` config; planner `sql_query` intent + `_do_sql` + `JobPlanner`; `POST /v1/db/*` + `atlas sql`. 645 tests. | ✅ done |
| **Sprint 20c** | OCR (image→text) | `atlas/ocr/engine.py` `OCRClient` + injectable `OCREngine`/`TesseractEngine` seam (lazy optional deps → **degrades gracefully** to `unavailable`; sandboxed sources; byte cap; honest outcomes, never raises); `OCRPlugin` (`ocr` cap: `ocr.image`, degraded-not-failed health); `OCRCapability` (`CAP_OCR`); `plugins.ocr.*` config + `pillow`/`pytesseract` deps; planner `ocr_image` intent + `_do_ocr` + `JobPlanner`; `POST /v1/ocr` + `atlas ocr`. 671 tests. | ✅ done |
| **Sprint 20d** | Email (read-only IMAP) | `atlas/mail/client.py` `MailClient` + injectable `MailBackend`/`IMAPBackend` seam (**read-only**: `readonly=True` + `BODY.PEEK`, no write path; **degrades gracefully** to `unavailable`/`unauthorized`; honest outcomes, never raises; pure header/body parsers); `MailPlugin` (`mail` cap: `search`/`message`/`folders`, degraded-not-failed health); `MailCapability` (`CAP_MAIL`); `plugins.mail.*` config + `ATLAS_MAIL_PASSWORD` secret; planner `mail_search` intent + `_do_mail` + `JobPlanner`; `POST /v1/mail/*` + `atlas mail`. 702 tests. | ✅ done |
| **Sprint 20e** | Browser (headless, read-only) | `atlas/browser/browser.py` `BrowserClient` + injectable `BrowserBackend`/`PlaywrightBackend` seam (**read-only** render+extract, no click/type/submit; lazy deps → **degrades gracefully** to `unavailable`; http(s)-only + robots via new `FetchClient.allowed` + timeout + sandboxed screenshots; honest outcomes, never raises); `BrowserPlugin` (`browser` cap: `open`/`screenshot`, degraded-not-failed health); `BrowserCapability` (`CAP_BROWSER`); `plugins.browser.*` config + optional `[browser]` Playwright extra; planner opt-in `browse_url` intent (keyword+URL) + `_do_browse` + `JobPlanner`; `POST /v1/browser/*` + `atlas browser`. 731 tests. **Tier-2 tool arc complete.** | ✅ done |
| **Sprint 21** | Autonomous research orchestration | `atlas/research/service.py` `ResearchService` (`research` cap, `research.run`) = the live **gather→verify→decide** loop: deterministic `query_plan` (scholar→web variants, capped) + pure `extract_value` (years skipped); folds sources into one `EvidenceGraph` claim (papers L3/L4 via `as_source()`, web L2), `verify_claim` (confidence + convergence), `decide(iteration, budget)` loops until stop (**convergence, not count**), then a verified §5a.5 report. Providers resolved **lazily** via capability registry → `unavailable` when absent, raising provider skipped, sources de-duped; honest `ok`/`empty`/`unavailable`/`error`, never raises. `ResearchCapability` (`CAP_RESEARCH`); `research.per_query` config; planner `research` intent + `_do_research` (+`research` gap) + `JobPlanner` (real claims flow into job reports — closes `_build_report` `claims=[]`); `POST /v1/research` + `atlas research`. 755 tests. **§5a research spine complete.** | ✅ done |
| **Sprint 22** | Hardening & Operability | Production-readiness pass, **no new capabilities**. `HealthStatus` gains a `degraded` tier (derived `level`, back-compat; scheduler/LLM report it); `Application.status()`/`uptime` + `GET /v1/status` + enriched `GET /v1/health` (severity+data). `atlas/kernel/preflight.py` (`check_config`/`probe_dependencies`/`worst_status`) + **`atlas doctor [--offline]`** (validate config + probe DB/LLM without starting workers). `SchedulerService` **graceful drain** (`scheduler.drain_timeout`: stop claiming, finish in-flight, then abandon→recover). Request middleware: access log + `http.requests`/`http.request.duration_ms` metrics + `X-Request-ID`. Generic `Exception` handler (structured 500 + request id) + `DatabaseError`→503 / `ConfigError`→500. First hermetic **end-to-end** test (`tests/test_integration.py`: objective→job→tools→verify→report). 773 tests. **Stage 2 complete.** | ✅ done |
| **Sprint 23** | Web Console | `atlas/web/` **zero-build, dependency-free SPA** (vanilla HTML/CSS/JS) served **same-origin** by the API — no Node/build, no CORS. `mount_ui(app, cfg)` mounts `atlas/web/static/` at `/ui` (`StaticFiles(html=True)`) + `/`→`/ui/`, gated by new `api.ui_enabled`. API-key login (validated vs `/v1/status`, `localStorage`, `Bearer` per call, 401→re-login). Views: **Chat** (sessions/history, tool-call chips, capability-gaps, linked citations), **Jobs** (create/list + live-polling detail with steps/progress/blocked/resume/cancel + inline report), **System** (`/v1/status` cards + `/v1/health` severity badges). XSS-safe `el()` DOM. Assets ship via hatchling package data; no new runtime dep. 779 tests. **Conversational frontend delivered — Stage 2 done incl. UI.** | ✅ done |
...
| ADR-0077 | 2026-07-12 | **Web Console — a zero-build same-origin SPA** (Stage 2 S23). The long-planned "conversational frontend": a visual surface so Atlas isn't CLI/HTTP-only. **Decision: no build toolchain.** Rather than a React/Vite/Node SPA, the console is **vanilla HTML/CSS/JS** (`atlas/web/static/index.html` + `styles.css` + `app.js`) served by the *same* FastAPI app — this matches the codebase's zero-heavy-deps pattern (argparse over Click, in-memory metrics over a Prometheus client) and, by being **same-origin**, needs **no CORS** and no dev server. `atlas/web/__init__.py` **`mount_ui(app, config)`** mounts the assets at **`/ui`** via `StaticFiles(directory=…, html=True)` and adds a `/`→`/ui/` redirect, all gated by a new **`api.ui_enabled`** flag (default true; false ⇒ neither the mount nor the redirect is registered, verified by a test). The static shell is **public** (it holds no secrets); the JS performs **API-key auth** in the browser — it collects the operator key once, validates it against `GET /v1/status`, stores it in `localStorage`, sends it as an `Authorization: Bearer` header on every `/v1` call, forces re-login on any 401, and clears it on "sign out". **Three views over the existing endpoints:** **Chat** — a sessions sidebar (`GET /v1/chat/sessions`) + "New chat", history load (`GET /v1/chat/sessions/{id}`), and a composer that posts to `POST /v1/chat` (Enter send / Shift+Enter newline) rendering the turn with **tool-call chips**, **capability-gap** warnings, and **citations** (hyperlinked when a source `url` is present); **Jobs** — create (`POST /v1/jobs`), list (`GET /v1/jobs`), and a detail pane that **live-polls every 2s while queued/running** showing the steps table + progress + blocked items, **Resume** (`/resume`) / **Cancel** (`/cancel`), and the finalized report rendered inline; **System** — `GET /v1/status` summary cards (version/uptime/severity counts) + `GET /v1/health` per-service **severity badges** (ok/degraded/failed), surfacing the S22 operability work. **Safety:** all rendering goes through a small `el(tag, attrs, …children)` helper that only ever sets `textContent` (never `innerHTML` with server data), so tool output, titles, and snippets cannot inject markup. The assets ship in the wheel automatically (hatchling includes package data under `atlas/`), so there is **no new runtime dependency** and no build artifact. Surfaced at `/ui` (and `/`); toggle with `api.ui_enabled`. Backend seam: because the UI is pure static + REST, it can later be swapped for a compiled SPA behind the same `/ui` mount without touching the server | ✅ Accepted |
| ADR-0076 | 2026-07-12 | **Hardening & Operability — production-readiness pass** (Stage 2 S22). With the feature set complete (S10–S21), S22 makes Atlas *operable and defensible* and adds **no new capabilities**; it closes the top gaps from a readiness audit. **(1) Health tiers.** `HealthStatus` (`atlas/services/base.py`) gains a **`degraded`** middle tier — a service that is *up but below full capability* (embedding model not pulled, some scheduler workers dead) — kept **fully back-compatible**: the `healthy` boolean is unchanged and a derived **`level`** (`ok`/`degraded`/`failed`) is computed from `severity` when set else from `healthy`, plus a `degraded_status()` constructor. The scheduler (partial-workers) and LLM (embed-not-pulled) now report it. **`Application.status()`** rolls up version + `uptime_seconds` + ok/degraded/failed counts; **`GET /v1/status`** exposes it and **`GET /v1/health`** is enriched with per-service `severity` + `data`; `atlas status` prints the tiered summary. **(2) `atlas doctor` preflight.** New `atlas/kernel/preflight.py` (`Check`/`check_config`/`probe_dependencies`/`run_preflight`/`worst_status`): pure **offline config validation** (API keys set, `scheduler.workers ≥ jobs.max_concurrent`, writable `paths.*`, valid `sandbox.backend`, `pg_dump` present when backups enabled) plus optional **dependency probes** that call the real DB + LLM `health_check` **without starting worker threads**; `atlas doctor [--offline]` prints ok/warn/fail and exits non-zero only on a hard failure. **(3) Graceful shutdown drain.** `SchedulerService.stop()` sets the stop flag (workers stop *claiming* immediately) then joins over a bounded **`scheduler.drain_timeout`** (default 30s) budget so an in-flight task can finish; anything still running past the budget is abandoned and left `running` (recovered on next boot) with a warning — replacing the old fixed 5s join-then-drop. **(4) Request observability.** A FastAPI HTTP middleware (`atlas/api/app.py`) records an access log, `http.requests` (method/route/status) + `http.request.duration_ms` metrics (keyed by **route template**, so cardinality stays bounded), and an **`X-Request-ID`** (honoured from the client or generated) echoed on every response. **(5) API resilience.** A generic **`Exception`** handler returns the *same* structured JSON as typed errors (`{error, detail, request_id}`) instead of a bare stack, and the error→status table maps **`DatabaseError`→503** (retryable) + `ConfigError`→500. **(6) First true end-to-end test.** `tests/test_integration.py` drives an objective through the **real** Job Engine → `AssistantService.run_step` dispatch → `ResearchService` gather→verify→decide loop → `VerificationEngine` → `ReportGenerator`, with an in-memory job repo + synchronous scheduler stand-in and only the external scholar/search edges faked (no DB/network/LLM). 773 tests (+18). **Stage 2 is complete**; optional follow-ons only (Web UI; JSON logs / API-key scopes / rate limiting when exposed beyond localhost) | ✅ Accepted |
| ADR-0075 | 2026-07-12 | **Autonomous research orchestration — the gather→verify→decide loop** (Stage 2 S21). The Stage-2 capstone: S13–S18a built the tools, S15 built the Verification Engine + Evidence Graph, S17 built the Report Generator — but *nothing drove them in a loop*, so `ResearchService` finally wires them into a self-directing researcher and closes the §5a research spine. New `atlas/research/service.py`: **`ResearchService`** = the `research` capability (`research.run`). Given an objective, `research(objective, budget?, max_iterations?)` runs a bounded loop: (1) **plan** — a *pure* `query_plan(objective)` interleaves scholar-then-web over query variants (`""`/`data`/`study`/`measurement`/`statistics`/`review`) after a `clean_objective` strips the research verb; the plan is filtered to the providers actually present so a missing provider never burns the budget, then capped at `max_search_iterations`; (2) **gather** — call the resolved `scholar`/`search` capability and fold each result into a *single* `EvidenceGraph` `Claim` as a graded `EvidenceItem` (papers keep their L3/L4 level via `Paper.as_source()`; web hits are L2 `technical`), extracting a numeric value per source with a *pure* `extract_value` (first non-year number, so convergence is meaningful) and de-duplicating sources by id across rounds; (3) **verify** — `VerificationEngine.verify_claim` recomputes calculated confidence + numeric convergence; (4) **decide** — `verify.decide(claim, iteration, budget)` returns stop/continue and the loop repeats until **stop** (Evidence Budget satisfied, convergence reached, or the iteration cap) — the stopping rule is **convergence, not a fixed source count** (§5a.4); (5) **report** — `ReportService.report(objective, graph, budget)` verifies + renders the §5a.5 scientific-review report. **Resolution is lazy and honest (R2/R3):** providers come from the shared `CapabilityRegistry` (scholar/search register later as plugins) or are injected directly for hermetic tests; no provider ⇒ **`unavailable`**, a provider that raises is caught and skipped, no evidence ⇒ **`empty`**, else **`ok`** — the loop never raises into the caller. The deterministic core (`query_plan`/`extract_value`/`clean_objective`) plus real `Paper`/`SearchHit`/Verification/Report make the whole loop testable offline. Concrete **`ResearchCapability`** contract (catalog `CAP_RESEARCH`, since S21); `research.per_query` config (sources per search). The planner gains a **`research`** intent — a research/investigate/deep-dive verb or "what does the evidence say", ordered so a plain `web_search` ("search the web for …") is *not* captured — with `AssistantService._do_research` (verdict + confidence + convergence + top sources surfaced as citations; `unavailable`→blocked step; `research` capability-gap fallback) and `JobPlanner` support, so a **research job step** flows *real* verified claims into the job's report (closing the `_build_report` `claims=[]` placeholder from S17). Bootstrap constructs the service once and hands it the `capabilities` registry by reference, registers it as an instance + capability + the `research.run` tool. Surfaced via `POST /v1/research` (`objective`, optional `max_iterations`) + `atlas research <objective> [--max-iterations]`. **This completes the §5a research spine**; remaining Stage-2 work is hardening + operability | ✅ Accepted |
| ADR-0074 | 2026-07-12 | **Browser — headless, read-only rendering as the last Tier-2 tool** (Stage 2 S20e). Deferred deliberately from the start of S20: it carries the heaviest dependency (Playwright + a real browser binary) and the largest safety surface, so it lands only after the deterministic, hermetic tools (Git/SQL/OCR/Email). It closes the retrieval loop as the **JS-rendering escalation from plain HTTP fetch** (§5c) — pages that only yield content after JavaScript runs. New `atlas/browser/browser.py`: **`BrowserClient`** renders a URL (or screenshots it) through an injectable **`BrowserBackend`** (default **`PlaywrightBackend`**, Chromium; all Playwright imports are lazy so the module always imports and the app always boots). **Read-only by design:** the backend only *navigates and extracts* — page title, `body` inner-text, and anchor `href`s — and there is **no click/type/submit/download** path at all. Like every other tool it **degrades gracefully and never raises** (R2/R3): a missing package/binary ⇒ `unavailable`, robots-disallowed ⇒ `blocked`, a slow nav ⇒ `timeout` (hard per-navigation budget), else `ok`/`empty`/`error` (a `_precheck` + per-method guards catch everything, even a crashing backend). **Safety rails:** `http(s)`-only; `robots.txt` is honoured by **reusing the shared net policy** — a new public **`FetchClient.allowed(url)`** exposes the existing robots logic so there is one implementation, not a copy; rendered text and the link list are capped; screenshots are confined to a **sandbox root** (default `paths.data/screenshots`, escape ⇒ `error`). **`BrowserPlugin`** = the `browser` capability with `browser.open(url)` (→ title/text/links/final_url/status) and `browser.screenshot(url, path, full_page)`; its `health_check` reports a missing engine as **degraded, not failed**. Concrete **`BrowserCapability`** contract (catalog `CAP_BROWSER`, since S20). The planner's **`browse_url`** intent is deliberately **opt-in** — a regex lookahead requires *both* a browser keyword (browse/browser/render/headless/screenshot/"open … in browser"/"javascript-rendered"/"dynamic page") *and* a URL, and the rule is ordered **before** `web_fetch`, so a bare URL still routes to plain fetch and the browser is only used on explicit escalation. `AssistantService._do_browse` renders a title + text preview + top links, maps `unavailable`/`blocked` to a blocked step, and reports the `browser` capability-gap honestly; `JobPlanner` accepts it. Config `plugins.browser.*` (`browser`/`headless`/`timeout`/`wait_until`/`max_text_chars`/`max_links`/`respect_robots`/`screenshot_root`); `atlas.plugins.browser_plugin` enabled (self-registers). **Playwright stays an *optional* dependency** — a new `[browser]` extra in `pyproject.toml` (`pip install -e '.[browser]' && playwright install chromium`); Atlas installs and runs without it, and the capability simply reports `unavailable`. Surfaced via `POST /v1/browser/open` + `POST /v1/browser/screenshot`; `atlas browser <url> [--screenshot <png>]`. **Backend seam:** the outcome contract lives in `BrowserClient`, so a Selenium/remote-CDP backend drops in behind `BrowserBackend` later without touching callers. **This completes the Stage-2 Tier-2 tool arc (S10–S20)**; LinkedIn is intentionally *not* a separate tool (no clean read-only API — it would be a browser recipe). Remaining Stage-2 work is autonomous multi-round research orchestration + hardening | ✅ Accepted |
| ADR-0073 | 2026-07-12 | **Email — read-only IMAP retrieval with secrets discipline** (Stage 2 S20d). The fourth Tier-2 tool: email becomes a research/assistant source (list folders, search a mailbox, open one message). New `atlas/mail/client.py`: **`MailClient`** works through an injectable **`MailBackend`** (default **`IMAPBackend`**, stdlib `imaplib` + `email` — zero extra deps, always importable). It is **read-only by construction** at the protocol level: mailboxes are selected with **`readonly=True`** (the server itself refuses writes), messages are fetched with **`BODY.PEEK[...]`** so opening a mail **never sets the `\Seen` flag** (reading is genuinely side-effect-free), and there is no STORE/DELETE/EXPUNGE/APPEND code path at all. Like Git/SQL/OCR it **degrades gracefully and never raises** (R2/R3): a `_guard` translates every backend failure into an honest outcome — `unavailable` (not configured, or the server can't be reached), `unauthorized` (credentials rejected), `error` (server error / unexpected crash), `empty` (no matches / unknown uid), else `ok`. Pure helpers decode RFC-2047 headers and extract a text/plain-preferring body (HTML stripped as a fallback, length-capped). **`MailPlugin`** = the `mail` capability with `mail.search(query, folder, limit)` (newest-first summaries), `mail.message(uid, folder)` (full message + body), `mail.folders()`; its `health_check` reports an unconfigured mailbox as **degraded, not failed** (`healthy=True` + `configured` flag). Concrete **`MailCapability`** contract (catalog `CAP_MAIL`, since S20). Planner gains a **`mail_search`** intent (routes `inbox`/`mailbox`, a read verb near "email(s)", or "email … from/about/subject …"; extracts a quoted or `for …`/`about …` query and an optional trailing `in <Folder>`) with `AssistantService._do_mail` (compact summary list; `unavailable`/`unauthorized`→blocked; honest `empty`/`error`; `mail` capability-gap fallback) and `JobPlanner` support. **Secrets discipline (Q7):** config `plugins.mail.*` holds host/port/username/`password_env`/ssl/folder/limits, but the **password is never in YAML/DB** — `build` reads it from the env var named by `password_env` (default `ATLAS_MAIL_PASSWORD`, added to `.env.example`); an empty password ⇒ the capability reports `unavailable`. `atlas.plugins.mail_plugin` enabled (self-registers). Surfaced via `POST /v1/mail/search` + `GET /v1/mail/folders` + `GET /v1/mail/message`; `atlas mail search|folders|message`. **Backend seam:** the outcome contract lives in `MailClient`, so a Gmail-API/JMAP backend drops in behind `MailBackend` later without touching callers. Browser automation (S20e) stays deliberately last; **LinkedIn folds into the browser** (no clean read-only API) | ✅ Accepted |
| ADR-0072 | 2026-07-12 | **OCR — image → text as a gracefully-degrading capability** (Stage 2 S20c). The third Tier-2 tool and the completion of the **Document Reader** (S13a explicitly deferred scanned/pixel text to "future OCR"). New `atlas/ocr/engine.py`: **`OCRClient`** reads a single image through an injectable **`OCREngine`** (default **`TesseractEngine`** = Pillow opens the file, pytesseract shells to the system `tesseract`). The distinguishing design point vs Git/SQL is a *system* dependency (the `tesseract` binary), so **all optional imports are lazy** and the engine must **degrade gracefully**: `available()` and typed `OCRUnavailable` mean a missing Pillow/pytesseract/binary surfaces as an `unavailable` outcome — the module always imports and the app always boots (verified by a test that runs the real engine and asserts `unavailable` when deps are absent). A `path` is an image resolved under and confined to a **sandbox root** (default `paths.documents`, like the filesystem plugin) with a per-image byte cap and a raster-suffix allow-list; escape/missing ⇒ `unavailable`, wrong-suffix/too-large ⇒ `unsupported`. Every call returns a plain dict with an `outcome` of `ok` | `empty` (no readable text) | `unsupported` | `unavailable` | `error` and **never raises** (R2/R3) — even a crashing engine is caught and mapped to `error`. **`OCRPlugin`** = the `ocr` capability with `ocr.image(path, lang=?)`; its `health_check` reports `healthy=True` with an `available` flag, treating a missing backend as **degraded, not failed**. Concrete **`OCRCapability`** contract (catalog `CAP_OCR`, since S20). Planner gains an **`ocr_image`** intent (the `ocr` keyword, "extract/read text from … image/screenshot/scan", or a bare `*.png|jpg|gif|bmp|tiff|webp` path — which by construction does *not* collide with the doc-ingest suffix set) with `AssistantService._do_ocr` (renders the extracted text; `unavailable`→blocked, honest `unsupported`/`empty`/`error`; `ocr` capability-gap fallback) and `JobPlanner` support. Config `plugins.ocr.*` (`root`/`lang`/`max_bytes`); `atlas.plugins.ocr_plugin` enabled (self-registers). `pillow`+`pytesseract` added to `pyproject.toml`/`requirements.txt`. Surfaced via `POST /v1/ocr` (`{path, lang?}`) + `atlas ocr <path> [--lang]`. **Engine seam:** the outcome contract lives in `OCRClient`, so an EasyOCR/cloud-OCR backend drops in behind `OCREngine` later without touching callers. Browser automation (deliberately last) + Email/LinkedIn remain as-needed | ✅ Accepted |
| ADR-0071 | 2026-07-12 | **SQL — read-only local database querying** (Stage 2 S20b). The second Tier-2 tool; Atlas can now query structured data, and a computed result set is **L5-quality evidence** (§5a.6), pairing with the Python sandbox (S16). New `atlas/sql/client.py`: **`SQLClient`** runs a *single* statement through an injectable **`SQLBackend`** (default **`SQLiteBackend`**, stdlib `sqlite3` — always available, zero-config, hermetic). It is **read-only by construction** via two independent layers: (1) a statement **guard** (`is_read_only`) that strips SQL comments, rejects multiple statements (an embedded `;`), requires the body to begin with `SELECT`/`WITH`/`EXPLAIN`/`VALUES`, and blocks any mutating/side-effecting keyword anywhere (`INSERT`/`UPDATE`/`DELETE`/`DROP`/`ALTER`/`CREATE`/`REPLACE`/`TRUNCATE`/`ATTACH`/`DETACH`/`VACUUM`/`REINDEX`/`GRANT`/`REVOKE`/`PRAGMA`/`BEGIN`/`COMMIT`/…); and (2) the SQLite backend opens the file with **`mode=ro`** (via `file:…?mode=ro` URI) as defence-in-depth, so even a guard bypass cannot write — verified by a test that calls the backend directly with a `DELETE`. A `source` is a db file resolved under and confined to a **sandbox root** (default `paths.data`, like the filesystem plugin); an escape or missing file is an honest `unavailable`. Every method (`query`/`tables`/`schema`) returns a plain dict with an `outcome` of `ok` | `empty` | `blocked` | `unavailable` | `error` and **never raises** (R2/R3); rows are capped by `max_rows` (with a `truncated` flag) and a soft per-query wall-clock timeout interrupts the connection from a watchdog thread. **`SQLPlugin`** = the `sql` capability with `sql.query` (columns + row dicts + truncated), `sql.tables` (tables/views), `sql.schema` (column defs). Concrete **`SQLCapability`** contract (catalog `CAP_SQL`, since S20). Planner gains a `sql_query` intent (fenced ```` ```sql ````, a bare `SELECT … FROM …`, or "query the database …"; extracts an optional `*.db`/`*.sqlite` source) with `AssistantService._do_sql` (renders a compact text table; honest blocked/unavailable/error) and `JobPlanner` support. Config `plugins.sql.*` (`root`/`default_source`/`max_rows`/`timeout`); `atlas.plugins.sql_plugin` enabled (self-registers). Surfaced via `POST /v1/db/query` + `GET /v1/db/tables` + `GET /v1/db/schema`; `atlas sql query|tables|schema`. **Backend seam:** the read-only contract lives in `SQLClient`, so a Postgres/other backend drops in behind the same `SQLBackend` protocol later without touching callers | ✅ Accepted |
